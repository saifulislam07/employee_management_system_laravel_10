                    ...
                    <tbody>
                        @if (count($users) > 0)
                            @foreach ($users as $key => $user)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td><img width="100" src="{{ asset('profile') }}/{{ $user->image }}" alt="">
                                    </td>
                                    <td>{{ $user->name }}</td>
                                    <td>{{ $user->email }}</td>
                                    ...
                                    <td>
                                        @if (isset(auth()->user()->role->permission['name']['user']['can-view']))
                                            <a href="{{ route('users.show', [$user->id]) }}"><i class="fas fa-desktop"
                                                    h></i></a>
                                        @endif
                                    </td>
                                    <td>
                                        @if (isset(auth()->user()->role->permission['name']['user']['can-edit']))
                                            <a href="{{ route('users.edit', [$user->id]) }}"><i class="fas fa-edit"
                                                    h></i></a>
                                        @endif
                                    </td>
                                    <td>
                                        @if (isset(auth()->user()->role->permission['name']['user']['can-delete']))
                                            <a href="#" data-toggle="modal"
                                                data-target="#removeUserModal{{ $user->id }}"><i
                                                    class="fas fa-trash"></i></a>
                                        @endif
                                        <!-- Modal -->
                                        <div class="modal fade" id="removeUserModal{{ $user->id }}" tabindex="-1"
                                            role="dialog" aria-labelledby="removeUserModalLabel" aria-hidden="true">
                                                ...
                                                <form action="{{ route('users.destroy', [$user->id]) }}" method="post">
                                                    ...
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="removeUserModalLabel">Delete User
                                                            </h5>
